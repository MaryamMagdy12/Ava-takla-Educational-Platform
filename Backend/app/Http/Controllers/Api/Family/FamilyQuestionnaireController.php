...
class FamilyQuestionnaireController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $paginator = StudentQuestionnaire::query()
            ->where('admin_interface', 'general_assembly')
        ...
        $responses = StudentQuestionnaireResponse::query()
            ->where('respondent_type', 'general_assembly')
            ->where('respondent_id', $family->id)
    ...
    {
        $this->ensureRespondentColumnsReady();
        $response = $this->engine->startOrResumeForActor($studentQuestionnaire, $request->user(), 'general_assembly');
    ...
    public function resume(Request $request, StudentQuestionnaireResponse $studentQuestionnaireResponse)
    {
        $this->ensureRespondentColumnsReady();
        abort_if(
            $studentQuestionnaireResponse->respondent_type !== 'general_assembly'
