...
class StudentQuestionnaireController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $responses = StudentQuestionnaireResponse::query()
            ->when(
                ...
                fn ($q) => $q->where('respondent_type', 'student')->where('respondent_id', $student->id),
    ...
    {
        $response = $this->engine->startOrResume($studentQuestionnaire, $request->user());
    ...
    public function resume(Request $request, StudentQuestionnaireResponse $studentQuestionnaireResponse)
    {
        if ($this->hasRespondentColumns()) {
            abort_if(
                $studentQuestionnaireResponse->respondent_type !== 'student'
                || (int) $studentQuestionnaireResponse->respondent_id !== (int) $request->user()->id,
                403
            );
        } else {
            abort_if((int) $studentQuestionnaireResponse->student_id !== (int) $request->user()->id, 403);
        }
