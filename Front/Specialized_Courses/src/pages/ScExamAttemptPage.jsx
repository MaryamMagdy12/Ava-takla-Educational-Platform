...
import ExamQuestionCard from '../components/sc-exams/ExamQuestionCard.jsx'
import ExamResultView from '../components/sc-exams/ExamResultView.jsx'
import '../assets/css/ScExamAttemptPage.css'
...
const ANSWERS_STORAGE_PREFIX = 'sc-exam-attempt-answers-v1'
...
export default function ScExamAttemptPage() {
  const { id } = useParams()
  ...
  const storageKey = useMemo(() => {
    ...
    const learnerId = String(user?.id ?? '')
    return `${ANSWERS_STORAGE_PREFIX}:${learnerId}:${examId}`
  }, [id, user?.id])
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        const existing = await specialExamApi.getExamResult(user, id)
        if (cancelled) return
        if (existing) {
        ...
        let payload = await specialExamApi.getExamAttempt(user, id)
        if (cancelled) return
        if (!payload) payload = await specialExamApi.startExam(user, id)
        if (cancelled) return
          ...
          if (raw) {
            ...
            if (parsed && typeof parsed === 'object') {
              const restoredAnswers =
                Array.isArray(parsed.answers) && parsed.answers.length === payload.exam.questions.length
                  ? parsed.answers.map((v) => normalizeStoredOption(v))
                  : defaultAnswers
  ...
  const onSubmit = useCallback(
    async (auto = false) => {
      ...
      setSubmitHint('')
      try {
        const final = await specialExamApi.submitExam(user, exam, answers)
        try {
    ...
    [allQuestionsAnswered, answers, exam, result, user],
  )
  ...
  if (!user?.id && !exam && !error && !result) {
    return (
      <section id="pg-exam-attempt-root" dir="rtl" lang="ar">
        <p className="pg-exam-attempt__loading">جارٍ تحميل الحساب...</p>
      </section>
    )
  }
            ...
            {exam.questions.map((_, qIndex) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="pg-exam-attempt__qmap-label">{answered ? 'تمت الإجابة' : 'لم تُجب'}</span>
                </button>
              )
            })}
          </div>
