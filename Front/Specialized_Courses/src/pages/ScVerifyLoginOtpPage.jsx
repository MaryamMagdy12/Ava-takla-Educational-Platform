...
export default function ScVerifyLoginOtpPage() {
  ...
  async function onSubmit(e) {
    ...
    setLoading(true)
    try {
      await verifyLoginOtp(email, otp.trim(), from)
    } catch (err) {
  ...
  async function resend() {
    ...
    setResendBusy(true)
    try {
      await postSpecialResendLoginOtp(email)
      setMsg('إن كان هناك تسجيل دخول قيد الانتظار لهذا البريد، أُرسل رمز جديد.')
    } catch (err) {
  ...
  return (
    <div id="pg-login-root">
      ...
      >
        <h1 className="pg-login__title">تأكيد تسجيل الدخول</h1>
        <p className="pg-login__hint">
          أُرسل رمز مكوّن من ٦ أرقام إلى <strong dir="ltr">{email}</strong> لإتمام الدخول بعد التحقق من كلمة المرور.
        </p>
