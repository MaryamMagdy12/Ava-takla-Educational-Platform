...
export default function QuestionnairesPage() {
  ...
  const load = async () => {
    try {
      const list = await contentApi.getQuestionnaires(student)
  ...
  const openQuestionnaire = async (row) => {
    ...
    try {
      const payload = row.response?.id
        ? await contentApi.getQuestionnaireResponse(student, row.response.id)
        : await contentApi.startQuestionnaire(student, row.id)
      setActive(payload)
      const initial = {}
      ...
      showToast({ type: 'error', message: e.message || 'تعذر فتح الاستبيان' })
    } finally {
      setBusy(false)
    }
  }
  ...
  const saveAnswers = async () => {
    ...
    try {
      ...
      await contentApi.saveQuestionnaireAnswers(student, active.response.id, payload)
      showToast({ type: 'success', message: 'تم حفظ الإجابات' })
    } catch (e) {
      showToast({ type: 'error', message: e.message || 'تعذر حفظ الإجابات' })
    } finally {
      setBusy(false)
    }
  }
  ...
  const submit = async () => {
    ...
    try {
      await saveAnswers()
      await contentApi.submitQuestionnaire(student, active.response.id)
      showToast({ type: 'success', message: 'تم إرسال الاستبيان بنجاح' })
      setActive(null)
      setAnswers({})
      await load()
    } catch (e) {
      showToast({ type: 'error', message: e.message || 'تعذر إرسال الاستبيان' })
    } finally {
      setBusy(false)
    }
  }
    ...
    <section id="pg-questionnaires-root">
      <div className="pg-questionnaires__shell">
        <div className="pg-questionnaires__hero-wrap">
          <PageHeader
            title="الاستبيانات"
            subtitle="أجب عن الاستبيانات المتاحة لك واحفظ إجاباتك ثم أرسلها."
            headerClassName="pg-questionnaires__hero"
            titleClassName="pg-questionnaires__title"
            subtitleClassName="pg-questionnaires__subtitle"
          />
                ...
                <div className="pg-questionnaires__card-actions">
                  {done ? (
                    <>
                      ...
                      <button type="button" className="pg-questionnaires__btn pg-questionnaires__btn--outline-muted" disabled>
                        تم الإرسال
                      </button>
                    </>
                  ) : (
